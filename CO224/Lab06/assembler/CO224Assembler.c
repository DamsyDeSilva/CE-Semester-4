 	...
 	while(fgets(line, LINE_SIZE, fi)!=NULL) // Read a line from the input .s file
	{
		...
		*************************************************************************/
		...
		strcasecmp(in_token,"swd")==0 || strcasecmp(in_token,"swi")==0 || strcasecmp(in_token,"j")==0) 
		{
			...
			in_token = strtok(NULL, delim); // Read the second token
			strcat(pline,in_token); strcat(pline,delim);// Write second token to the pre-processed line, with a delimiter
			strcat(pline,"X "); // Write 'X' to the pre-processed line (for the ignored bits 15-8), with a delimiter
			if(j) // If this is a 'j' instruction, need to ignore bits 7-0
				strcat(pline,"X"); // Write 'X' to the pre-processed line
			else // Otherwise
			{	
				in_token = strtok(NULL, delim); // Read the third token 
				strcat(pline,in_token);// Write third token to the pre-processed line
			}
			while(1)
			{
